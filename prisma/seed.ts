  ...
  await Promise.all(individualTreesData.map(t =>
    prisma.tree.create({
      data: {
        id: t.id,
        species: t.species,
        status: (t.status as unknown as string).replace('Recién Plantado','Recien_Plantado').replace('Necesita Poda','Necesita_Poda') as TreeStatus,
        streetName: t.streetName,
        streetNumber: t.streetNumber,
        sidewalk: (t.sidewalk as unknown as { lat: number; lng: number } | null) ?? null,
        location: (t.location as unknown as { lat: number; lng: number } | null) ?? null,
